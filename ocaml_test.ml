(* let issue_num = Sys.argv.(1) in
    let issue_contents = Sys.argv.(2) in
        let _ = (Printf.printf "%s and %s\n" issue_num issue_contents) in
            ...
            let sim_header = Cohttp.Header.of_list [("X-RapidAPI-Key", "your_key_here"); ("X-RapidAPI-Host", "twinword-text-similarity-v1.p.rapidapi.com"); ("content-type", "application/x-www-form-urlencoded")]
